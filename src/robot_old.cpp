...
Robot::Robot(ros::NodeHandle& input) {
	...
	Link link3(1.57, 0.0, 0.0, 1.57, 1.0, "src/safety_index/meshes/a0509_blue_stl/link3.stl");
	Link link4(-1.57, 0.0, 0.367, 0.0, 1.0, "src/safety_index/meshes/a0509_blue_stl/link4.stl");
	Link link5(1.57, 0.0, 0.0, 0.0, 1.0, "src/safety_index/meshes/a0509_blue_stl/link5.stl");
	Link link6(0.0, 0.0, 0.124, 0.0, 1.0, "src/safety_index/meshes/a0509_blue_stl/link6.stl");
	RobotGripper grip(0.0, 0.0, 0.0, -1.57, 1.0, "src/safety_index/meshes/other/gripperr.stl");
...
void Robot::UpdateRobotMesh(){
	...
	*(this->mBaseTriangularMesh) = vector<Triangle>(); // Per cancellare la mesh già presente
	...
	for(it_t = this->GetTriangularMesh()->begin(); it_t != this->GetTriangularMesh()->end(); ++it_t) {
...
void Robot::ComputeSafetyInd(){
	...
	vector<Triangle> triangles = *(this->mBaseTriangularMesh);
	...
	vector<double> c_x;
	vector<double> c_y;
	vector<Eigen::Vector3d> ref_origins;

	//Defining the geometrical characteristics of each triangle (Polverini et al. Cumulative Safety Field)
	for (i=0; i<n_triangles; i++){

		Eigen::Vector3d dir_AB, seg_AB, seg_AC;

		point_A = triangles[i].GetPoint1();
		point_C = triangles[i].GetPoint3();

		seg_AB = (*triangles[i].GetPoint2() - *point_A);
		dir_AB = seg_AB / seg_AB.norm();
		seg_AC = *point_C - *point_A;

		// Compute projection of C on segment AB (point P)
		double proj_len = seg_AC.dot(dir_AB); // length of segment AP
		Eigen::Vector3d point_P = *point_A + dir_AB * proj_len ; // actual projection (point P)
		ref_origins.push_back(point_P); // the point P is the origin of the ref. frame fixed to the triangle
		
		Eigen::Vector3d seg_PC = *point_C - point_P;
		Eigen::Vector3d seg_PA = *point_A - point_P;

		// Coordinates in the triangle frame: origin in P, x along PC and y along PB
		double coord_xC = seg_PC.norm();
		double coord_yA = seg_PA.norm();

		// coord_yA is negative if P is inside segment AB
		...
		double coeff_x = -2.0 / 3.0 * coord_xC;
		double coeff_y = -2.0 / 3.0 * (coord_yA + coord_yB);

		c_0.push_back(coeff_0);
		c_x.push_back(coeff_x);
		c_y.push_back(coeff_y);
	...
	for (i = 0; i < n_triangles; i++) {
		...
		point_A = triangles[i].GetPoint1();
		point_B = triangles[i].GetPoint2();
		point_C = triangles[i].GetPoint3();

		x_axis = (* point_C - ref_origins[i]) / (*point_C - ref_origins[i]).norm(); // x is defined as the direction from P to C
		y_axis = (* point_B - *point_A) / (*point_B - *point_A).norm(); // y is defined as the direction from A to B
		z_axis = x_axis.cross(y_axis);

		if(i==200 || i==1146 || i==2964 || i==3612 || i==4681 || i==5580 || i==6078 || i==13397){
			ROS_INFO("%f,%f,%f", x_axis(0),x_axis(1),x_axis(2));
			ROS_INFO("%f,%f,%f", y_axis(0),y_axis(1),y_axis(2));
			ROS_INFO("%f,%f,%f", z_axis(0),z_axis(1),z_axis(2));
		}

		// Sometimes it happens that point C coincides with the origin of the reference, or that two points
		// (A and B) of a triangle coincide. This IF allows to avoid errors due to these situations.
		if ((*point_C - ref_origins[i]).norm() != 0 && (*point_B - *point_A).norm() != 0){

			// For each body point, compute its distance to the origin of the i-th triangle frame and increment CSF
			for (j = 0; j < n_points; j++) {
				dist_point = body_points[j] - ref_origins[i];
				...
				this->csf = this->csf + c_0[i] + c_x[i] * dist_x + c_y[i] * dist_y + dist_point_n * dist_point_n;
